...
export default function LoginSlr() {
  ...
  const handleLogin = async () => {
    ...
    if (!id  || !password) return alert('Semua field wajib diisi')

    if (error || !data) return alert('ID tidak ditemukan')
    ...
    if (match) {
      ...
      localStorage.setItem('seller_id', data.id) // simpan info login
      ...
      <div className='flex containerAuth'>
        ...
        <div className='basis 1/6 flex flex-col container2Auth'>
          ...
          <div className='text-center'>
            <p className='text-sm'>Belum memiliki akun? <a href="/registerslr" className="font-bold text-sm text-[#3A4D39] hover:underline">
              Daftar
            </a></p>
          </div>
        </div>
